...
static apt_bool_t mrcp_client_message_handler(mrcp_connection_t *connection, mrcp_message_t *message, apt_message_status_e status, const char *pszInPartMsg)
{
	if(status == APT_MESSAGE_STATUS_COMPLETE) {
		...
		if (NULL == message || NULL == pszInPartMsg) {
			return TRUE;
		}
		/* message is inCompletely parsed */
		mrcp_control_channel_t *channel;
		apt_str_t identifier;		
		apt_str_t sid = message->channel_id.session_id;
		apt_str_t rName = message->channel_id.resource_name;
		apt_id_resource_generate(&message->channel_id.session_id, &message->channel_id.resource_name, '@', &identifier, message->pool);
		channel = mrcp_connection_channel_find(connection,&identifier);
		if(channel) {
			mrcp_connection_agent_t *agent = connection->agent;
			int msgLen = strlen(pszInPartMsg);
			char *buf = apr_palloc(message->pool, msgLen + 1);
			memcpy(buf, pszInPartMsg, msgLen);
			buf[msgLen] = '\0';
...
static apt_bool_t mrcp_client_poller_signal_process(void *obj, const apr_pollfd_t *descriptor)
{
	...
	do {
		...
		if(mrcp_client_message_handler(connection, message, msg_status, stream->text.buf) == FALSE) {
			return FALSE;
		}
	}
